...
You'll also need `Rcpp`, which compiles C++ functions and makes specificity 10 to 20 times faster. Apparently it's difficult to install on OS X, and (maybe?) more difficult on OS X Catalina. I didn't know this and wrote the whole package on Ubuntu Linux, where it's VERY easy to install Rcpp. I'm looking into making a pre-compiled version available for Macs, but don't hold your breath. In any case, see the Rcpp install notes here: https://teuder.github.io/rcpp4everyone_en/020_install.html. I'm also looking into breaking the package into two parts: specificity and specificity_cpp. The former will suggest the latter, and use C++ functions if they are available. As it stands now, you'll need a working `Rcpp` installation to install `specificity`. 
